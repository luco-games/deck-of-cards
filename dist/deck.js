...
var Deck = (function () {
  ...
  var intro = {
    deck: function deck(_deck2) {
      _deck2.intro = _deck2.queued(intro);
    ...
    card: function card(_card3) {
      ...
      _card3.shuffle = function (cb) {
        ...
        _card3.animateTo({
          ...
          x: plusMinus(Math.random() * 40 + 20) * fontSize$1 / 16,
          y: -z,
          rot: 0
        });
        _card3.animateTo({
  ...
  var sort = {
    deck: function deck(_deck4) {
      _deck4.sort = _deck4.queued(sort);
    ...
    card: function card(_card4) {
      ...
      _card4.sort = function (i, len, cb, reverse) {
        var z = i / 4;
        var delay = i * 10;
  ...
  var flip = {
    deck: function deck(_deck5) {
      _deck5.flip = _deck5.queued(flip);

      function flip(next, side) {
        var flipped = _deck5.cards.filter(function (card) {
          return card.side === 'front';
        }).length / _deck5.cards.length;

        _deck5.cards.forEach(function (card, i) {
  ...
  function Card(i) {
    ...
    // self = card
    var self = { i: i, rank: rank, pos: i, $el: $el, mount: mount, unmount: unmount, setSide: setSide };
    ...
    self.setRank = function (rank) {
      $el.setAttribute('class', 'card rank' + rank);
    };
  ...
  Deck.ease = ease;
  Deck.modules = { fan: fan, intro: intro, shuffle: shuffle, sort: sort, flip: flip };
  Deck.Card = Card;
  Deck.prefix = prefix;
